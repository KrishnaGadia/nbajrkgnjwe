    ...
    private static class Client_Thread extends Thread {
		...
		// define a protocol
                ...
                while (true) {
                    ...
                    System.out.println("Obtained "+ input+" from "+username);
                    if (input == null) {
                        return;
                    }
								if(input.charAt(0) == 'U'){
								//System.out.println("Got a UI Message");
								for (PrintWriter writer : print_writer) { 
                        writer.println(input);
                        }
                     }//UI Message, just echo
                 else{
                 		if(input.equalsIgnoreCase("Interrupt")){ 
											input = "Please don't Draw, I want to lead the session";
										}
		                  for (PrintWriter writer : print_writer) { 
