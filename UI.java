...
public class UI {
  ...
  static String username;
	static JButton red_button,blue_button,yellow_button,black_button,white_button,green_button,pink_button,cyan_button,choose_color,fill,tiny,big,eraser_button,textbox,clear,freehand,line_button,rectangle_button,circle_button,triangle_button,square_button,token_button;
  static JPanel panel;    
    ...
    file.add(quit);     
		...
		token_button.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				try{
				//username = JOptionPane.showInputDialog(null,"Please enter your name: ","Get Username",JOptionPane.PLAIN_MESSAGE);		
				UI.writeToServer("T,0,0,0,0,",outToServer,inFromServer);		
...
if(token){
	...
	UI.writeToServer("X,0,0,0,0,",outToServer,inFromServer);
      }}
    });
...
if(token){
	...
	UI.writeToServer("C,"+drawColor.getRGB()+",0,0,0,",outToServer,inFromServer);
      }}
    });
...
if(token){
 ...
 // TRIANGLE Implementation : Mode -5
    ...
    panel.addMouseListener(new MouseAdapter(){
      ...
      {
        g.drawLine(prevX,prevY,x,y);
				UI.writeToServer("L,"+prevX+","+prevY+","+x+","+y+",",outToServer,inFromServer);
      ...
      else if(mode==2) //Circle
      {
	      ...
	      g.drawOval(mx,my,dx,dy);
				UI.writeToServer("O,"+mx+","+my+","+dx+","+dy+",",outToServer,inFromServer);
      }
      ...
      else if(mode==3) //Square
      {
        ...
        g.drawRect(mx,my,dx,dx);
				UI.writeToServer("R,"+mx+","+my+","+dx+","+dx+",",outToServer,inFromServer);
      ...
      else if(mode==4) //Rectangle
      {
        ...
        g.drawRect(mx,my,dx,dy);
				UI.writeToServer("R,"+mx+","+my+","+dx+","+dy+",",outToServer,inFromServer);
      ...
      else if(mode == 5) // Triangle
      {
        ...
        g.drawLine((int)((x+prevX)/2),prevY,x,y);  
				UI.writeToServer("L,"+prevX+","+y+","+x+","+y+",",outToServer,inFromServer);
				UI.writeToServer("L,"+prevX+","+y+","+(int)((x+prevX)/2)+","+prevY+",", outToServer, inFromServer);
				UI.writeToServer("L,"+(int)((x+prevX)/2)+","+prevY+","+x+","+y+",", outToServer, inFromServer);	  
...
if(token){
        ...
        g.setColor(drawColor);
        g.fillRect(0,0,1000,700);
				UI.writeToServer("F,0,0,0,0,",outToServer,inFromServer);
        drawColor = Color.black; // setting it back to default color - BLACK
        UI.writeToServer("C,"+Color.black.getRGB()+",0,0,0,",outToServer,inFromServer);
      }}
    });
	...
	public static void writeToServer(String set, PrintWriter outToServer,BufferedReader inFromServer) {
	  ...
	  if(ServerInput.charAt(0)=='T'){
		token=false; // Coordinator has no rights to draw/write on the whiteboard as it is giving the token to the pupil who requested the token
		
	  // Disabling the buttons - Coordinator will not have the rights to draw/write on the Whiteboard
			...
			public void mouseEntered(MouseEvent e){
				try{
					...
					//System.out.println("Co-od: " + ServerInput);
						if(ServerInput.charAt(0) == '#' && ServerInput.charAt(3)!='#')
							System.exit(0);
						...
						switch(type){
							case 'R': g.drawRect(v1,v2,v3,v4);
											break;
							case 'O': g.drawOval(v1,v2,v3,v4);
											break;
							case 'L': g.drawLine(v1,v2,v3,v4);
											break;
							case 'C': g.setColor(new Color(v1));
											break;
							case 'X': Color tem = g.getColor();
											g.setColor(Color.white);
        							g.fillRect(0,0,1000,700);
        							g.setColor(tem);
											break;
							case 'F': Color t = g.getColor();
        							g.fillRect(0,0,1000,700);
        							g.setColor(t);
											break;
							case 'Z' : token=true; 
